...
describe('ControlChannel message handling', () => {
  ...
  it('acknowledges cmd.load when the asset is available locally', async () => {
    ...
    expect(finalProgress).toEqual(['tone', 1024, 1024]);
    const finalState = useSessionStore.getState();
    expect(finalState.assets.has('tone')).toBe(true);
    expect(finalState.assetProgress.tone?.loaded).toBe(1024);
  });

  it('reports errors when cmd.load asset is missing locally', async () => {
    const invalidate = vi.fn();
    const unload = vi.fn();
    const seek = vi.fn();
    const removeBuffer = vi.fn();
    ...
    expect(errors).toContain('asset "tone" is not available locally');
    const ack = JSON.parse(dc.send.mock.calls.at(-1)?.[0] as string);
    expect(ack.payload).toEqual({
      ok: false,
      forTxn: 'txn-load',
      error: 'asset "tone" is not available locally',
    });
    const finalProgress = setAssetProgressSpy.mock.calls.at(-1);
    expect(finalProgress).toEqual(['tone', 0, 1024]);
    const finalState = useSessionStore.getState();
